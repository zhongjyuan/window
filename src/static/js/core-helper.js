/**
 * Created by Helper on 2016/12/9.
...
var Helper = {
			...
			if (obj === null) {
				...
				if (obj instanceof Array) {
					...
					for (var i = 0, len = obj.length; i < len; i++) {
						o.push(Helper.clone(obj[i]));
					}
				} else {
					o = {};
					for (var j in obj) {
						o[j] = Helper.clone(obj[j]);
					}
				}
			}
		} else {
		...
		for (var attr in jsonObject2) {
			resultJsonObject[attr] =
				...
				typeof jsonObject2[attr] === "object"
					? Helper.jsonMerge(resultJsonObject[attr], jsonObject2[attr], false)
					: jsonObject2[attr];
		}
	...
	jsonp: function (url, data, callback, name_data, name_callback) {
		...
		if (!Helper.jsonp_funcs) {
			Helper.jsonp_funcs = {};
		}
		Helper.jsonp_funcs[func_name] = callback;
		var rel = "";
		rel += url;
		if (url.indexOf("?") < 0) {
			rel += "?";
		...
		rel +=
			name_callback +
			"=" +
			encodeURIComponent('Helper.jsonp_funcs["' + func_name + '"]');
		if (data) {
			var data_str = JSON.stringify(data);
			data_str = encodeURIComponent(data_str);
			rel += "&" + name_data + "=" + data_str;
		}
		Helper.loadScript(rel, function (script) {
