ZHONGJYUANA.render = function (data) {
	//监听消息
	$(window).on("message", function (e) {
		var winVerify = function (from) {
			var id = from[0];
			var secrete = from[1];
			var win = ZHONGJYUANA.vue.wins[id];
			if (win) {
				if (win.secrete === secrete) {
					return true;
				} else {
					ZHONGJYUANA.vue.winClose(id);
					ZHONGJYUANA.msg(
						ZHONGJYUANA.lang("SecurityRisk"),
						ZHONGJYUANA.lang("SecurityRiskDetail")
					);
				}
			}
			return false;
		...
		var from = data.from;
		var id = from[0];
		var wins = ZHONGJYUANA.vue.wins;
		var win = wins[id];
		if (!winVerify(from)) return;
		switch (data.type) {
			case "zhopngjyuan-pong":
				win.pong = Date.now();
				break;
			case "zhopngjyuan-eval":
				var rel = ZHONGJYUANA.child[data.eval.method].call(
					ZHONGJYUANA.child,
					data.eval.data,
					id
				);
	...
	document.body.onbeforeunload = function (event) {
		if (!ZHONGJYUANA.static.beforeOnloadEnable) {
			return;
		}
		var rel = ZHONGJYUANA.lang("BeforeUnload");
		if (!window.event) {
		...
		methods: {
			contextMenuAddTo: function (objCopy) {
				...
				var funcMsg = function () {
					ZHONGJYUANA.util.simpleMsg(ZHONGJYUANA.lang("Added"));
				};
				var menuAddToTile = [];
				this.tiles.forEach(function (group, n) {
					menuAddToTile.push([
						ZHONGJYUANA.util.getStrFa("square") + group.title,
						function () {
							ZHONGJYUANA.addTile(objDeepCopy, n);
							funcMsg();
						},
						!ZHONGJYUANA.static.changeable,
					]);
				});
				return [
					...
					[
						[
							...
							function () {
								ZHONGJYUANA.addShortcut(objDeepCopy);
								funcMsg();
							},
							!ZHONGJYUANA.static.changeable,
						],
						[
							...
							function () {
								ZHONGJYUANA.addMenuItem(objDeepCopy);
								funcMsg();
							},
							!ZHONGJYUANA.static.changeable,
						],
						[
							...
							function () {
								ZHONGJYUANA.addSidebarBtn(objDeepCopy);
								funcMsg();
							},
							!ZHONGJYUANA.static.changeable,
						],
						...
						"<span style='color: darkgray'>" +
							ZHONGJYUANA.util.getStrFa("square") +
							ZHONGJYUANA.lang("Tiles") +
							"</span>",
			...
			contextMenuUninstall: function (appId) {
				...
				return [
					...
					function (v) {
						ZHONGJYUANA.util.simpleConfirm(
							Yuri2.template(
								ZHONGJYUANA.lang("UninstallConfirm"),
								that.apps[appId].title
							),
							function () {
								if (ZHONGJYUANA.uninstall(appId)) {
									ZHONGJYUANA.util.simpleMsg(
										ZHONGJYUANA.lang("UninstallCompleted")
									);
								}
							}
						);
			...
			contextMenuOpen: function (obj) {
				...
				return [
					...
					[
						[
							ZHONGJYUANA.util.getStrFa("window-restore") +
								ZHONGJYUANA.lang("Normal"),
							function (v) {
								that.appOpen(
									obj.app,
						...
						[
							ZHONGJYUANA.util.getStrFa("window-maximize") +
								ZHONGJYUANA.lang("Maximize"),
							function (v) {
								that.appOpen(
									obj.app,
						...
						[
							ZHONGJYUANA.util.getStrFa("window-minimize") +
								ZHONGJYUANA.lang("Minimize"),
							function (v) {
								that.appOpen(
									obj.app,
						...
						[
							ZHONGJYUANA.util.getStrFa("paper-plane") +
								ZHONGJYUANA.lang("Outer"),
							function (v) {
								that.appOpen(
									obj.app,
			...
			initRuntime: function (first) {
				...
				if (first) {
					this.$nextTick(function () {
						//jq插件 滚动条
						var box = [
							"#zhongjyuan .tiles-box",
							"#zhongjyuan .startMenu .menu",
							"#zhongjyuan .center .msgs",
						];
						...
						$("#zhongjyuan").css({ opacity: 1, display: "block" }); //显示
					});
				}
					...
					setInterval(function () {
						...
						if (now > itv + last) {
							...
							if (that.configs.wallpaperSlideRandom) {
								index = ZHONGJYUANA.util.randInt(0, len - 1); //随机取下标
							} else {
								//顺序取下标
								index = indexLast + 1;
								if (index >= len) {
				...
				if (Yuri2.isIE()) {
					...
					if (ZHONGJYUANA.static.WarningPerformanceInIE) {
						setTimeout(function () {
							ZHONGJYUANA.msg(
								ZHONGJYUANA.lang("WarningPerformanceInIETitle"),
								ZHONGJYUANA.lang("WarningPerformanceInIEContent")
							);
						}, 2000);
					}
				}
				...
				if (first) {
					...
					setInterval(function () {
						try {
							var exportJson = JSON.stringify(ZHONGJYUANA.export());
							if (exportJson !== lastExport) {
								lastExport = exportJson;
								localStorage.setItem(
									ZHONGJYUANA.static.localStorageName,
									exportJson
								);
				...
				if (first) {
					var f5_check = function (e) {
						...
						if (e.ctrlKey && (e.which || e.keyCode) === 116) {
							// 强制刷新，清除缓存
							ZHONGJYUANA.static.beforeOnloadEnable = false;
							return;
						}
				...
				if (first) ZHONGJYUANA.onReady(autoRun);
				else {
					autoRun();
				}

				//社区版提示检测
				if (first && !ZHONGJYUANA.static.serialNumber) {
					ZHONGJYUANA.onReady(function () {
						setTimeout(function () {
							ZHONGJYUANA.msg(
								"zhongjyuan window v" + ZHONGJYUANA.info.version + " 社区版",
								"当前ZHONGJYUAN为社区版，仅限个人用户学习使用，禁止商用及企业使用。<br/>更多信息及获取商业授权请访问:<br/>ZHONGJYUAN官网：" +
									'<a style="color: white" target="_blank" href="https://zhongjyuan.yuri2.cn">https://zhongjyuan.yuri2.cn</a><p>欢迎使用支付宝扫描二维码对作者进行捐赠，表达您对ZHONGJYUAN的支持~<br/><br/><img style="width: 100%" src="./res/img/donation.png"/></p>'
							);
						}, 1500);
					});
				}
			...
			flash: function () {
				...
				that.runtime.customTileRandomToken = ZHONGJYUANA.util.randInt(
					1000,
					9999
				);
			},
			appOpen: function (id, options, source) {
				...
				if (this.runtime.winOpened >= this.configs.openMax) {
					ZHONGJYUANA.util.simpleMsg(
						Yuri2.template(
							ZHONGJYUANA.lang("MaxWinsReached"),
							this.configs.openMax
						)
					);
					return;
				}
				...
				var evalNum = function (str) {
					...
					if (isNaN(str)) {
						try {
							...
							var query =
								"var x=ZHONGJYUANA.vue.runtime.clientSize.width;var y=ZHONGJYUANA.vue.runtime.clientSize.height-40;" +
								str;
							rel = eval(query);
						} catch (e) {
							ZHONGJYUANA.debug(e);
				...
				win.style.index = ZHONGJYUANA.util.getBiggerWinZIndex();
				win.url = ZHONGJYUANA.util.urlParams(win.url, win.params, win.hash);
				if (win.urlRandomToken)
					win.url = ZHONGJYUANA.util.updateUrlRandomToken(win.url);
				...
				that.$nextTick(function () {
					...
					ZHONGJYUANA.util.iframeOnClick.track(
						ifr,
						function () {
							if (
								Object.getOwnPropertyNames(
									ZHONGJYUANA.util._iframe_click_lock_children
								).length === 0
							) {
			...
			winMaximize: function (id) {
				...
				var win = this.wins[id];
				if (!win.resizable) {
					ZHONGJYUANA.util.simpleMsg(ZHONGJYUANA.lang("AttentionNoResize"));
					return;
				}
				win.state = "max";
			...
			winRefresh: function (id) {
				...
				this.winJump(
					id,
					win.urlRandomToken
						? ZHONGJYUANA.util.updateUrlRandomToken(target)
						: target
				);
			},
			winHome: function (id) {
			...
			winContextMenu: function (id, e) {
				...
				var menu = [
					[
						ZHONGJYUANA.util.getStrFa("refresh") + ZHONGJYUANA.lang("Refresh"),
						function () {
							that.winRefresh(id);
						},
					],
				...
				if (!win.addressBar) {
					menu.push([
						ZHONGJYUANA.util.getStrFa("location-arrow") +
							ZHONGJYUANA.lang("DisplayAddressBar"),
						function () {
							win.addressBar = true;
						},
					]);
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("location-arrow") +
							ZHONGJYUANA.lang("HideAddressBar"),
						function () {
				...
				menu.push([
					ZHONGJYUANA.util.getStrFa("magic") +
						ZHONGJYUANA.lang("CollectAsApplication"),
					function () {
						layer.prompt(
							{
								title: ZHONGJYUANA.lang("AttentionEnterAppName"),
								formType: 0,
							...
							function (input, index) {
								...
								app.url = ZHONGJYUANA.util.removeUrlRandomToken(win.url);
								app.title = input;
								app.icon.type = "str";
								app.icon.content = input;
								var appId = "app-" + ZHONGJYUANA.util.getID();
								ZHONGJYUANA.addApp(appId, app);
								ZHONGJYUANA.addShortcut(appId); //自动添加到桌面
								ZHONGJYUANA.addMenuItem(appId); //自动添加到菜单
								ZHONGJYUANA.util.simpleMsg(ZHONGJYUANA.lang("Added"));
							}
						);
					},
				...
				var menuWin = [];
				menuWin.push(
					ZHONGJYUANA.util.getStrFa("arrows") +
						Yuri2.template(
							"${0}px * ${1}px",
				...
				menuWin.push([
					ZHONGJYUANA.util.getStrFa("window-restore") +
						ZHONGJYUANA.lang("CentreAdjustment"),
					function () {
				...
				if (that.winIsMin(id)) {
					menuWin.push([
						ZHONGJYUANA.util.getStrFa("clone") +
							ZHONGJYUANA.lang("DisplayWindow"),
						function () {
							that.winShow(id);
						},
					]);
				...
				if (win.style.position.left) {
					menuWin.push([
						ZHONGJYUANA.util.getStrFa("angle-double-right") +
							ZHONGJYUANA.lang("AlignRight") +
					...
					menuWin.push([
						ZHONGJYUANA.util.getStrFa("angle-double-left") +
							ZHONGJYUANA.lang("AlignLeft") +
							Yuri2.template(" (${0}px)", parseInt(win.style.position.x)),
						function () {
							win.style.position.left = !win.style.position.left;
						},
				...
				if (win.style.position.top) {
					menuWin.push([
						ZHONGJYUANA.util.getStrFa("angle-double-down") +
							ZHONGJYUANA.lang("AlignBottom") +
					...
					menuWin.push([
						ZHONGJYUANA.util.getStrFa("angle-double-up") +
							ZHONGJYUANA.lang("AlignTop") +
							Yuri2.template(" (${0}px)", parseInt(win.style.position.y)),
						function () {
							win.style.position.top = !win.style.position.top;
						},
				...
				menu.push([
					ZHONGJYUANA.util.getStrFa("windows") +
						ZHONGJYUANA.lang("WindowPosition"),
					menuWin,
				]);
				menu.push([
					ZHONGJYUANA.util.getStrFa("paper-plane") +
						ZHONGJYUANA.lang("OpenOuter"),
					function () {
						window.open(win.url);
					},
				]);
				...
				if (win.source) {
					menu.push([
						ZHONGJYUANA.util.getStrFa("crop") +
							ZHONGJYUANA.lang("RememberSizeAndPosition"),
						function () {
							win.source.style = {};
							...
							ZHONGJYUANA.util.simpleMsg(ZHONGJYUANA.lang("Recorded"));
						},
						!ZHONGJYUANA.static.changeable,
					]);
					if (win.source.style) {
						menu.push([
							ZHONGJYUANA.util.getStrFa("crop") +
								ZHONGJYUANA.lang("ResetSizeAndPosition"),
							function () {
				...
				if (win.plugin) {
					...
					if (win.background) {
						menu.push([
							ZHONGJYUANA.util.getStrFa("sort-up") +
								ZHONGJYUANA.lang("PutForeground"),
							function () {
						...
						menu.push([
							ZHONGJYUANA.util.getStrFa("sort-down") +
								ZHONGJYUANA.lang("PutBackground"),
							function () {
								win.background = true;
							},
						]);
				...
				menu.push("|");
				menu.push([
					ZHONGJYUANA.util.getStrFa("close") + ZHONGJYUANA.lang("Close"),
					function () {
						that.winClose(id);
					},
				]);
			...
			desktopContextMenu: function (e) {
				...
				var menu = [
					...
					[
						ZHONGJYUANA.util.getStrFa("desktop") +
							ZHONGJYUANA.lang("DisplayDesktop"),
						function () {
							that.showDesktop();
						},
					],
					[
						ZHONGJYUANA.util.getStrFa("credit-card") +
							ZHONGJYUANA.lang("Personalization"),
						[
							[
								ZHONGJYUANA.util.getStrFa("paint-brush") +
									ZHONGJYUANA.lang("ThemeColor"),
								function () {
									ZHONGJYUANA.open("zhongjyuan-system", {
										data: { nav: "colors" },
									});
								},
								!ZHONGJYUANA.static.changeable,
							...
							[
								ZHONGJYUANA.util.getStrFa("picture-o") +
									ZHONGJYUANA.lang("Wallpaper"),
								function () {
									ZHONGJYUANA.open("zhongjyuan-system", {
										data: { nav: "wallpaper" },
									});
					...
					[
						ZHONGJYUANA.util.getStrFa("cog") +
							ZHONGJYUANA.lang("SystemOptions"),
						function () {
							ZHONGJYUANA.open("zhongjyuan-system");
						},
						!ZHONGJYUANA.static.changeable,
					...
					"|",
					[
						ZHONGJYUANA.util.getStrFa("object-group") +
							ZHONGJYUANA.lang("FullScreen"),
						function () {
							ZHONGJYUANA.util.enableFullScreen();
						},
					],
					[
						ZHONGJYUANA.util.getStrFa("object-ungroup") +
							ZHONGJYUANA.lang("NormalScreen"),
						function () {
							ZHONGJYUANA.util.disableFullScreen();
						},
					],
					[
						ZHONGJYUANA.util.getStrFa("circle-o-notch") +
							ZHONGJYUANA.lang("Reload"),
						function () {
							ZHONGJYUANA.f5();
						},
					],
					"|",
					[
						ZHONGJYUANA.util.getStrFa("info-circle") +
							ZHONGJYUANA.lang("AboutUs"),
						function () {
							ZHONGJYUANA.aboutUs();
						},
					],
			...
			shortcutOpen: function (s, id) {
				...
				ZHONGJYUANA.debug(ZHONGJYUANA.lang("OpenIcons") + ":" + s.title);
			},
						...
						if (dragTo.over) {
							...
							if (!s.children && !sOn.children) {
								...
								that.$set(that.shortcuts, dragTo.id, {
									title:
										ZHONGJYUANA.lang("IconGroup") + ZHONGJYUANA.util.getID(),
									drag: {
										mDown: false,
										left: 0,
										top: 0,
			...
			shortcutContextMenu: function (id, pid, e, isDrawer) {
				...
				if (s.children) {
					menu.push([
						ZHONGJYUANA.util.getStrFa("tags") + ZHONGJYUANA.lang("Rename"),
						function () {
							that.shortSetting = s;
						},
						!ZHONGJYUANA.static.changeable,
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("sitemap") + ZHONGJYUANA.lang("UnGroup"),
						function (v) {
							s.children.forEach(function (t) {
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("tags") +
							ZHONGJYUANA.lang("Rename") +
							"/" +
							ZHONGJYUANA.lang("Options"),
						function (v) {
							that.shortSetting = s;
						},
						!ZHONGJYUANA.static.changeable,
				...
				if (s.app && that.apps[s.app].badge) {
					menu.push([
						ZHONGJYUANA.util.getStrFa("map-pin") +
							ZHONGJYUANA.lang("ClearSuperscript"),
						function () {
				...
				menu.push([
					...
					function (v) {
						ZHONGJYUANA.util.simpleConfirm(
							Yuri2.template(ZHONGJYUANA.lang("DeleteIconConfirm"), s.title),
							function () {
								if (isDrawer) {
									that.shortcuts[pid].children.splice(id, 1);
								} else {
			...
			barContextMenu: function (e) {
				...
				var menu = [
					[
						ZHONGJYUANA.util.getStrFa("window-minimize") +
							ZHONGJYUANA.lang("HideAll"),
						function () {
							that.winHideAll();
						},
					],
					[
						ZHONGJYUANA.util.getStrFa("clone") + ZHONGJYUANA.lang("DisplayAll"),
						function () {
							that.winShowAll();
						},
					],
					[
						ZHONGJYUANA.util.getStrFa("window-close") +
							ZHONGJYUANA.lang("CloseAll"),
						function () {
							that.winCloseAll();
						},
					],
				...
				if (that.configs.topTaskBar) {
					menu.push(
						[
							ZHONGJYUANA.util.getStrFa("arrow-down") +
								ZHONGJYUANA.lang("BottomTaskBar"),
							function () {
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("arrow-up") +
							ZHONGJYUANA.lang("TopTaskBar"),
						function () {
			...
			tileContextMenu: function (groupIndex, tileIndex, e) {
				...
				var menu = [
					...
					"|",
					[
						ZHONGJYUANA.util.getStrFa("braille") +
							ZHONGJYUANA.lang("Small") +
							ZHONGJYUANA.lang("Size"),
						function () {
							tile.w = 1;
							tile.h = 1;
						},
				...
				if (that.tiles.length > 1) {
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("cut") + ZHONGJYUANA.lang("MoveTo"),
						moveMenu,
					]);
				}
				...
				menu.push([
					...
					function () {
						ZHONGJYUANA.util.simpleConfirm(
							Yuri2.template(ZHONGJYUANA.lang("DeleteTileConfirm"), tile.title),
							function () {
								that.tiles[groupIndex].data.splice(tileIndex, 1);
							}
						);
			...
			tileSrcCustom: function (app) {
				return app.urlRandomToken
					? ZHONGJYUANA.util.updateUrlRandomToken(
							app.customTile,
							"",
			...
			menuContextMenu: function (data) {
				...
				if (item.app && !item.children) {
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("object-group") +
							ZHONGJYUANA.lang("NewSubgroup"),
						function () {
							var itemNew = {
								title: ZHONGJYUANA.lang("Group"),
								children: {},
								open: true,
							};
				...
				menu.push("|");
				menu.push([
					ZHONGJYUANA.util.getStrFa("cut") + ZHONGJYUANA.lang("CutAndExchange"),
					function () {
						that.runtime.menuItemCut = itemId;
						ZHONGJYUANA.util.simpleMsg(
							item.title + " " + ZHONGJYUANA.lang("Recorded")
						);
					},
					!ZHONGJYUANA.static.changeable,
				]);
				if (that.runtime.menuItemCut) {
					menu.push([
						ZHONGJYUANA.util.getStrFa("paste") + ZHONGJYUANA.lang("Paste"),
						function () {
					...
					menu.push([
						ZHONGJYUANA.util.getStrFa("exchange") +
							ZHONGJYUANA.lang("Exchange"),
						function () {
							var idA = itemId,
								childA,
								fatherA;
				...
				menu.push([
					...
					function () {
						//递归删除依赖菜单
						ZHONGJYUANA.util.simpleConfirm(
							Yuri2.template(ZHONGJYUANA.lang("DeleteMenuConfirm"), item.title),
							function () {
			...
			backgroundToThemeColor: function () {
				var that = this;
				ZHONGJYUANA.util.imgUrlToThemeColor(
			...
			backgroundUpdateScale: function () {
				...
				ZHONGJYUANA.util.imgUrlToSize(url, function (size) {
			...
			sidebarBtnContextMenu: function (i, e) {
				...
				var menu = [
					...
					"|",
					[
						ZHONGJYUANA.util.getStrFa("arrow-up") + ZHONGJYUANA.lang("ShiftUp"),
						function () {
					...
					[
						...
						function () {
							ZHONGJYUANA.util.simpleConfirm(
								Yuri2.template(ZHONGJYUANA.lang("DeleteBtnConfirm"), btn.title),
								function () {
				...
				var menu = [
					...
					[
						...
						function () {
							ZHONGJYUANA.util.simpleConfirm(
								Yuri2.template(
									ZHONGJYUANA.lang("DeleteGroupConfirm"),
									group.title
								),
								function () {
			...
			btnShortSettingAdvanceClick: function (appId) {
				...
				ZHONGJYUANA.open("zhongjyuan-system", { data: { appSetting: appId } });
			},
		...
		watch: {
			...
			"startMenu.open": {
				handler: function (val, oldVal) {
					...
					if (val) {
						this.$nextTick(ZHONGJYUANA.util.artificiallyResize);
					}
				},
			},
			"startMenu.menu": {
