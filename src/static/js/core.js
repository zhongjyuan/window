window.ZHONGJYUANA = {
	info: {
		softwareName: "zhongjyuan window",
		version: "1.0.0",
		iconBtnStart: "html5",
		author: "zhongjyuan@outlook.com",
		contactInformation: "zhongjyuan@outlook.com",
		officialWebsite: "zhongjyuan.club",
		welcome: "本网站UI由 zhongjyuan window 强力驱动\n更多信息：zhongjyuan.club",
		copyrightDetail: "copyright@2023 zhongjyuan.club",
		otherStatements: "接下来过的每一天,都是余生中最年轻的一天",
	},
	data: {},
	lang: function (key) {
		return ZHONGJYUANA.static.languages[ZHONGJYUANA.static.lang]
			? ZHONGJYUANA.static.languages[ZHONGJYUANA.static.lang][key]
			: "";
	},
	readyStack: [],
	loadStack: [],
	_baseData: function () {
		return {
			...
			runtime: {
				...
				customTileRandomToken: ZHONGJYUANA.util.randInt(1000, 9999), //自定义磁贴iframe的随机token
	...
	init: function (data) {
		...
		data = data
			? data
			: localStorage.getItem(ZHONGJYUANA.static.localStorageName)
			? JSON.parse(localStorage.getItem(ZHONGJYUANA.static.localStorageName))
			: this._baseData();
		data = ZHONGJYUANA.format(data);
		$("#loading").remove();
		ZHONGJYUANA.render(data);
		//ready调用
	...
	aboutUs: function () {
		//关于我们
		this.open("zhongjyuan-system", {
			data: { nav: "aboutUs" },
		});
	},
	uninstall: function (appId) {
		if (Yuri2.inArray(ZHONGJYUANA.static.lockedApps, appId)) {
			this.util.simpleMsg(
				ZHONGJYUANA.lang("UninstallFailed") +
					ZHONGJYUANA.lang("AppLockedCanNotChange")
			);
			return false;
		}
	...
	addApp: function (appId, data) {
		...
		app.url = ZHONGJYUANA.util.removeUrlRandomToken(app.url);
	...
	addTileGroup: function (groupTitle) {
		var v = this.vue;
		var title = groupTitle ? groupTitle : ZHONGJYUANA.util.getID();
	...
	debug: function (log) {
		!ZHONGJYUANA.static.debug || console.log(log);
	},
	onLoad: function (cb) {
	...
	loadRes: function () {
		...
		document.getElementById("loading-lang-init").innerHTML = ZHONGJYUANA.lang(
			"LoadingInitializing"
		);
		document.getElementById("loading-powered-by").innerHTML =
			ZHONGJYUANA.lang("LoadingPoweredBy");
		document.title = ZHONGJYUANA.static.softwareName;
		var flagFifty = false; //解决圆环动画断裂
		var itvLoading = setInterval(function () {
			var percent = parseInt((counterLoad / maxLoad) * 100);
			document.getElementById("on-load-file-name").innerHTML = ZHONGJYUANA
				.static.debug
				? onLoading
				: ZHONGJYUANA.lang("LoadingStandby");
		...
		/**  加载资源 */
		const versionTail = "?v=" + ZHONGJYUANA.info.version;
		//jq
		ZHONGJYUANA.util.loadScript(
		...
		ZHONGJYUANA.util.loadStyle(
			pathRes + "/css/main.css" + versionTail,
			function () {
				onLoading = "main.css";
				counterLoad++;
			}
		...
		ZHONGJYUANA.util.loadStyle(
			pathRes + "/css/zhongjyuan-layer-skin.css" + versionTail,
			function () {
				onLoading = "zhongjyuan-layer-skin.css";
				counterLoad++;
			}
		);
		ZHONGJYUANA.util.loadStyle(
		...
		ZHONGJYUANA.util.loadStyle(
			pathRes + "/css/tiles.css" + versionTail,
			function () {
				onLoading = "tiles.css";
				counterLoad++;
			}
		...
		ZHONGJYUANA.util.loadScript(
			pathRes + "/js/Yuri2.js" + versionTail,
			function () {
				onLoading = "Yuri2.js";
				counterLoad++;
			}
		...
		ZHONGJYUANA.util.loadPrefetch(
			pathRes + "/apps/element-ui/index.css",
			function () {
		...
		ZHONGJYUANA.util.loadPrefetch(
			pathRes + "/apps/element-ui/index.js",
			function () {
		...
		ZHONGJYUANA.util.loadScript(
			...
			function () {
				...
				ZHONGJYUANA.util.loadScript(
					pathRes + "/js/core-icon-vue-component.js" + versionTail,
					function () {
						onLoading = "core-icon-vue-component.js";
						counterLoad++;
					}
				);
				ZHONGJYUANA.util.loadScript(
					pathRes + "/js/zhongjyuan-vue-components.js" + versionTail,
					function () {
						onLoading = "zhongjyuan-vue-components.js";
						counterLoad++;
					}
				);
			}
		...
		ZHONGJYUANA.util.loadScript(
			pathRes + "/js/core-render.js" + versionTail,
			function () {
				onLoading = "core-render";
				counterLoad++;
			}
		);
		ZHONGJYUANA.util.loadScript(
			pathRes + "/js/core-io.js" + versionTail,
			function () {
				onLoading = "core-io";
				counterLoad++;
			}
	...
	util: {
		...
		isTrustedApp: function (appId) {
			return Yuri2.inArray(ZHONGJYUANA.static.trustedApps, appId);
		},
		...
		simpleConfirm: function (msg, cb) {
			var cfm = layer.confirm(
				msg,
				{ skin: "zhongjyuan", zIndex: 19930010 },
				function () {
					layer.close(cfm);
					cb();
				}
		...
		getID: function () {
			return window.ZHONGJYUANA.vue.configs.idCounter++ + "";
		},
		...
		dataCopy: function (name) {
			//从vue的data中获得对象的深拷贝
			var data = name ? ZHONGJYUANA.vue.$data[name] : ZHONGJYUANA.vue.$data;
		...
		getAppDataTemplate: function () {
			return {
				...
				icon: {
					type: "fa",
					content: "wpforms",
					bg: ZHONGJYUANA.util.getRandomColor(),
				},
				openMode: "normal",
				plugin: false,
				position: {
		...
		getAppByWinId: function (winId) {
			var app = ZHONGJYUANA.vue.wins[winId].app;
			if (typeof app === "string") {
				app = ZHONGJYUANA.vue.apps[app];
			}
			return app;
		},
		_winZIndexMax: 0,
		...
		hasApp: function (item) {
			...
			return this.isSet(ZHONGJYUANA.vue.apps[id]);
		},
	},
	child: {
		//用于被子窗口调用的函数
		getID: function () {
			return ZHONGJYUANA.util.getID();
		},
		getVersion: function () {
			return ZHONGJYUANA.info.version;
		},
		refresh: function (id, idSelf) {
			id = id || idSelf;
			ZHONGJYUANA.vue.winRefresh(id);
		},
		setWinData: function (data, id) {
			//设置窗口的某参数
			var v = ZHONGJYUANA.vue;
			if (!v.wins[id]) {
				return;
			}
			for (var i in data) {
		...
		setWallpaper: function (urlImg, id) {
			return ZHONGJYUANA.setWallpaper(urlImg);
		},
		setThemeColor: function (color, id) {
			return ZHONGJYUANA.setThemeColor(color);
		},
		open: function (data, id) {
			return Yuri2.isArray(data)
				? ZHONGJYUANA.open(data[0], data[1])
				: ZHONGJYUANA.open(data);
		},
		close: function (params, id) {
			ZHONGJYUANA.vue.winClose(params ? params : id);
		},
		minimize: function (params, id) {
			ZHONGJYUANA.vue.winMinimize(params ? params : id);
		},
		maximize: function (params, id) {
			ZHONGJYUANA.vue.winMaximize(params ? params : id);
		},
		hide: function (params, id) {
			ZHONGJYUANA.vue.winMinimize(params ? params : id);
		},
		show: function (params, id) {
			ZHONGJYUANA.vue.winShow(params ? params : id);
			ZHONGJYUANA.vue.winSetActive(params ? params : id);
		},
		restore: function (params, id) {
			ZHONGJYUANA.vue.winRestore(params ? params : id);
		},
		msg: function (data) {
			ZHONGJYUANA.msg(data[0], data[1]);
		},
		simpleMsg: function (data) {
			ZHONGJYUANA.util.simpleMsg(data);
		},
		setAppBadge: function (data, id) {
			var v = ZHONGJYUANA.vue;
			var app = Yuri2.isArray(data)
				? v.apps[data[0]]
				: ZHONGJYUANA.util.getAppByWinId(id);
		...
		getAppVersion: function (data, id) {
			var app = data ? data : ZHONGJYUANA.util.getAppByWinId(id);
			return app ? app.version : null;
		},
		setup: function (data, id) {
			ZHONGJYUANA.util.simpleConfirm(
				ZHONGJYUANA.lang("ChildMethodSetupConfirm"),
				function () {
					var v = ZHONGJYUANA.vue;
					//子页安装应用、图标、侧边栏、菜单、磁贴
					if (data.apps) {
					...
					if (data.shortcuts) {
						data.shortcuts.forEach(function (t) {
							if (ZHONGJYUANA.util.hasApp(t)) ZHONGJYUANA.addShortcut(t);
						});
					}
					if (data.sidebar) {
						data.sidebar.forEach(function (t) {
							if (ZHONGJYUANA.util.hasApp(t)) ZHONGJYUANA.addSidebarBtn(t);
						});
					}
					if (data.tiles) {
						data.tiles.forEach(function (t) {
							if (ZHONGJYUANA.util.hasApp(t)) ZHONGJYUANA.addTile(t);
						});
					}
					if (data.menu) {
						var hasError = false;
						var addMenuAttrOpen = function (item) {
							...
							if (item.children) {
								...
								if (!ZHONGJYUANA.util.hasApp(item)) {
									hasError = true;
								}
							}
						};
					...
					var win = v.wins[id];
					ZHONGJYUANA.msg(
						ZHONGJYUANA.lang("ChildMethodSetupSuccessMsgTitle"),
						ZHONGJYUANA.lang("ChildMethodSetupSuccessMsgContent") + win.url
					);
				}
			);
		},
		uninstall: function (data, id) {
			//卸载一个或多个应用
			ZHONGJYUANA.util.simpleConfirm(
				ZHONGJYUANA.lang("ChildMethodUninstallConfirm"),
				function () {
					var list = [];
					if (typeof data === "string") list.push(data);
					else list = data;
					list.forEach(function (t) {
						ZHONGJYUANA.uninstall(t);
					});
					var win = v.wins[id];
					ZHONGJYUANA.msg(
						ZHONGJYUANA.lang("ChildMethodUninstallSuccessMsgTitle"),
						ZHONGJYUANA.lang("ChildMethodUninstallSuccessMsgContent") + win.url
					);
				}
			);
		},
		import: function (data, id) {
			if (!ZHONGJYUANA.util.isTrustedApp(ZHONGJYUANA.vue.wins[id].app)) {
				return false;
			}
			ZHONGJYUANA.import(data);
			return true;
		},
		...
		historyBack: function (data, id) {
			var winId = data || id;
			var win = ZHONGJYUANA.vue.wins[winId];
			var h = win.history;
...
ZHONGJYUANA.util.onloadSafe(function () {
	ZHONGJYUANA.util.loadScript("./configs.js", function () {
		if (ZHONGJYUANA.static.lang.toLocaleLowerCase() === "auto") {
			ZHONGJYUANA.static.lang = (
		...
		var fnLoadResFromText = function (text) {
			var json;
			json = eval("(" + text + ")");
			ZHONGJYUANA.static.languages[ZHONGJYUANA.static.lang] = json;
			ZHONGJYUANA.loadRes();
		};
		ZHONGJYUANA.util.loadContentFromUrl(
			"./langs/" + ZHONGJYUANA.static.lang + ".json",
			"GET",
			function (err, text) {
				if (!err) {
					fnLoadResFromText(text);
				} else {
					ZHONGJYUANA.static.lang = "en";
					ZHONGJYUANA.util.loadContentFromUrl(
						"./langs/" + ZHONGJYUANA.static.lang + ".json",
						"GET",
						function (err, text) {
