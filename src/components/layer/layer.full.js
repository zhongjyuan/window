...
!(function (window, undefined) {
	...
	var layer = {
		...
		ready: function (callback) {
			...
			isLayui
				? layui.addcss(
						"modules/layer/default/layer.css?v=" + layer.v + ver,
						callback,
						cssname
				  )
				: layer.link(
						"skin/default/layer.css?v=" + layer.v + ver,
						callback,
						cssname
				  );
			return this;
		...
		callback(
			[
				...
				config.shade
					? '<div class="layer-shade" id="layer-shade' +
					  ...
					  ("z-index:" +
							...
							(config.shade[0] * 100 || config.shade * 100) +
							");") +
					  '"></div>'
					: "",
				...
				'<div class="' +
					...
					(config.id || "") +
					'" class="layer-content' +
					(config.type == 0 && config.icon !== -1 ? " layer-padding" : "") +
					(config.type == 3 ? " layer-loading" + config.icon : "") +
					'">' +
					(config.type == 0 && config.icon !== -1
						? '<i class="layer-ico layer-ico' + config.icon + '"></i>'
						: "") +
					...
					(config.btn
						? (function () {
								...
								return (
									'<div class="' +
									doms[6] +
									(config.btnAlign ? " layer-btn-" + config.btnAlign : "") +
									'">' +
									button +
									"</div>"
								);
			...
			$.extend(
				{
					...
					title: (function () {
						var len = tab.length,
							...
							str = "";
						if (len > 0) {
							str = '<span class="layer-tabnow">' + tab[0].title + "</span>";
							for (; ii < len; ii++) {
