ZHONGJYUANA.onReady(function () {
	var lang =
		ZHONGJYUANA.static.lang === "zh-cn" || ZHONGJYUANA.static.lang === "zh-tw"
			? {
					Mon: "星期一",
					MonShort: "一",
					Tues: "星期二",
	...
	var oDays = oCalendar
		.getElementsByClassName("_days")[0]
	...
	function showDate() {
		...
		if (_setmonth === 0) {
			str =
				_fn_layout(_setyear - 1, 11, _setdate) +
				_fn_layout(_setyear, _setmonth, _setdate) +
				_fn_layout(_setyear, _setmonth + 1, _setdate);
		} else if (_setmonth === 11) {
			str =
				_fn_layout(_setyear, _setmonth - 1, _setdate) +
				_fn_layout(_setyear, _setmonth, _setdate) +
				_fn_layout(_setyear + 1, 0, _setdate);
		} else {
			str =
				_fn_layout(_setyear, _setmonth - 1, _setdate) +
				_fn_layout(_setyear, _setmonth, _setdate) +
	...
	_aStrong[0].onclick = function () {
		...
		if (onOff === 0) {
			...
			_temp_ten = 0;
			_aStrong[0].innerHTML =
				Math.floor(_setyear / 10) * 10 +
				"-" +
				(Math.floor(_setyear / 10) * 10 + 9);
			_oYears_months.style.display = "none";
			_oNormal.style.display = "none";
	...
	function ten(_year) {
		_oTenYears_ul.innerHTML =
			_layout_tenyear(_year - 10) +
			_layout_tenyear(_year) +
			_layout_tenyear(_year + 10);

		for (var i = 0; i < _oTenYears_lis.length; i++) {
			_oTenYears_lis[i].onclick = function () {
				for (var i = 0; i < _oTenYears_lis.length; i++) {
					_oTenYears_lis[i].className === "active" &&
						(_oTenYears_lis[i].className = "");
				}
	...
	_r_l[0].onclick = function () {
		if (onOff === 0) {
			...
			_aStrong[0].innerHTML =
				Math.floor(_temp / 10) * 10 + "-" + (Math.floor(_temp / 10) * 10 + 9);
			ten(_temp);
