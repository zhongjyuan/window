/**
 * contextMenu v2.2.4
 * @author Helper(yuri2peter@qq.com)
 ...
 */
window.ContextMenu = {
	_className: "zhongjyuan-context-menu",
			...
			if (item === "|") {
				...
				ul.append(
					$(
						'<li><div class="title" title="' +
							ContextMenu._getMainContent(item) +
							'">' +
							item +
							"</div></li>"
					)
				...
				var sub = $(
					'<li><div class="title ' +
						(item[2] === true ? "disable" : "") +
						'" title="' +
						ContextMenu._getMainContent(item[0]) +
						'">' +
						item[0] +
						"</div></li>"
				);
				...
				if (typeof item[1] === "object") {
						...
						if (t === "|") {
							...
							var subLi = $(
								'<li><div class="title ' +
									(t[2] === true ? "disable" : "") +
									'" title="' +
									ContextMenu._getMainContent(t[0]) +
									'">' +
									t[0] +
									"</div></li>"
							);
					...
					if (
						y + dom.height() > document.body.clientHeight &&
...
$(document).click(function (e) {
	if (
		$(e.target).hasClass(ContextMenu._className) ||
		$(e.target).parents("." + ContextMenu._className).length > 0
	)
		return;
	ContextMenu._removeContextMenu();
});
