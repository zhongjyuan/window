		...
		var tpl =
			...
			fillHSXFields = function (hsx, cal) {
				$(cal)
					.data("colpick")
					.fields.eq(4)
					.val(Math.round(hsx.h))
					.end()
					.eq(5)
					.val(Math.round(hsx.s))
					.end()
					.eq(6)
					.val(Math.round(hsx.x))
					.end();
			},
			fillHexFields = function (hsx, cal) {
				$(cal)
					.data("colpick")
					.fields.eq(0)
					.val($(cal).data("colpick").hsl ? hslToHex(hsx) : hsbToHex(hsx));
			},
			...
			setSelector = function (hsx, cal) {
				$(cal)
					...
					.selector.css(
						"backgroundColor",
						"#" +
							($(cal).data("colpick").hsl
								? hslToHex({ h: hsx.h, s: 100, x: 50 })
				...
				$(cal)
					...
					.selectorIndic.css({
						...
						top: parseInt(
							($(cal).data("colpick").height * (100 - hsx.x)) / 100,
							10
						),
					});
			},
					...
					.hue.css(
						...
						parseInt(
							$(cal).data("colpick").height -
								($(cal).data("colpick").height * hsx.h) / 360,
							10
						)
					);
			},
			...
			change = function (ev) {
				...
				if (this.parentNode.className.indexOf("_hex") > 0) {
					cal.data("colpick").color = col = cal.data("colpick").hsl
						? hexToHsl(fixHex(this.value))
						: hexToHsb(fixHex(this.value));
			...
			downIncrement = function (ev) {
				...
				var current = {
					...
					max:
						this.parentNode.className.indexOf("_hsx_h") > 0
							? 360
							: this.parentNode.className.indexOf("_hsx") > 0
							? 100
							: 255,
					y: ev.pageY,
					field: field,
				...
				change.apply(
					current.cal
						...
						.val(
							parseInt(
								(360 *
									(current.cal.data("colpick").height - (pageY - current.y))) /
				...
				return this.each(function () {
					...
					if (!$(this).data("colpickId")) {
						...
						cal.addClass(
							"colpick_" +
								options.layout +
								(options.submit ? "" : " colpick_" + options.layout + "_ns")
						);
						...
						if (ngIE) {
							...
							huebar.attr(
								"style",
								"background:-webkit-linear-gradient(top center," +
									stopList +
